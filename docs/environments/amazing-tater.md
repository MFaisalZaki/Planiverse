...
| `blocks` | a bundled one's | blocks, drawn from single squares, dominoes and 2×2s |
...
(`PROFILES`, one per room): the bounding box inside its walls, its blocks, pits, turnstiles and
taters, and the share of the box that is wall. The generator's block shapes are single squares,
dominoes and 2×2s, where the cartridge also has 1×5s, 4×3s and L-shapes, so a generated room
is stocked like a real one without being built from the same pieces. The method is
...
https://pcgbook.com/); the test is
breadth-first search, the same `solve` runs.
