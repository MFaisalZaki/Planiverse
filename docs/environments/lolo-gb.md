...
serialised to bytes), so search can branch by rewinding the machine, at the cost of an expansion
that takes a save-state load, a button press and a settle rather than a few microseconds.
