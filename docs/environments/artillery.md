...
![BFWS solving artillery instance 0](../renders/artillery.gif)

BFWS's plan for instance `0`: 2 moves, one frame per state. The same plan is also a
[contact sheet](../renders/artillery.png), every frame on one image, captioned with the step
...
result = IteratedBFWS(max_width=1000, progress=measures.artillery).solve(env)
trace = env.simulate(result.plan)

env.render_trace(trace, "artillery.gif")                                   # animated
env.render_trace(trace, "artillery.png", actions=result.plan, env=env)     # contact sheet
```

Every frame is the state's own text: the field as a height map, with the gun, the wind and the targets still standing. See [docs/rendering.md](../rendering.md) for the other output formats.
