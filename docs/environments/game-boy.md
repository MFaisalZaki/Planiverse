...
No cartridge ships with this repository, so the picture we can publish is of the one the test
suite assembles for itself: `tests/counter_rom.py`, an original program that counts button
presses and draws the count as a tile in the top left corner, running under PyBoy's generic
wrapper with its default goal of surviving twenty actions. BFWS's plan for instance `0`: 20
moves, one console frame per state, found in 20 expansions and seven seconds. The
[contact sheet](../renders/game_boy.png) keeps twelve of the twenty-one frames, captioned with
the step number, the action that produced it, and a note on the goal state. With a cartridge
of your own the frames are the game's own screen.
...
env = GameBoyEnv("/path/to/counter.gb")      # or any cartridge, see below
env.set_index(0)
env.reset()
...
attribute is read as a drop, which is how "game over, or a lost life" is written. A state
where the dead end holds is a dead end whether or not the goal holds too. Relative specs
(`delta`, `drop`) are measured from the instance's own initial state, after any opening the
generator played, not from the stage's first frame.
...
through; `render(target)` and `render_trace` write a trace as a GIF, a contact sheet or a
directory of PNGs of those frames, each captioned with its step, the action that produced it
and whether it is a goal or a dead end. `charts=False` and the other `render_trace` options
apply as elsewhere; the state's text (the tile grid and the fields) is `str(state)`.
