...
![BFWS solving puzznic_gb instance 0](../renders/puzznic_gb.gif)

BFWS's plan for instance `0`: 11 moves, one frame per state. The same plan is also a
[contact sheet](../renders/puzznic_gb.png), captioned with the step number, the action that
