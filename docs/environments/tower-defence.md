...
BFWS's plan for instance `0`: 7 moves, one frame per state. The same plan is also a
[contact sheet](../renders/tower_defence.png), every frame on one image, captioned with the step
...
Every frame is the state's own text: the wave fought, the lives and gold left, and the towers built. See [docs/rendering.md](../rendering.md) for the other output formats.
