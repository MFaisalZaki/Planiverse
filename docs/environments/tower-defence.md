...
![BFWS solving tower_defence instance 0](../renders/tower_defence.gif)

BFWS's plan for instance `0`: 7 moves, drawn as the readings of each state over the plan: the
lives and gold left, the towers built of each kind, and the waves fought. A frame is the chart up
to its state, so the GIF grows a step at a time. The same plan is also a
[sheet](../renders/tower_defence.png), the whole plan on one figure, with the action that produced
...
env.render_trace(trace, "tower_defence.gif")                                   # animated
env.render_trace(trace, "tower_defence.png", actions=result.plan, env=env)     # contact sheet
```
