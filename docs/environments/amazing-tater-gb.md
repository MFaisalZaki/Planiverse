...
actual behaviour; the two agree on every transition we have compared, so the choice is a trade
between fidelity and the four orders of magnitude in speed described under [Cost](#cost).
...
`str(state)` prints the friendly board. `render_trace` supplies this environment's own cartridge,
so the frames are real console screenshots:
