...
look at. The cost of that restriction is that a plan needing a distant substation is out of reach;
`PowerGridEnv(restrict_to_overloads=False)` offers the lot.
...
`str(state)` describes the grid in a few lines. `render_trace` typesets it into a contact sheet,
PDF, GIF or directory of PNGs:
