...
BFWS's plan for instance `0`: 1 moves, drawn as the readings of each state over the plan: the
worst line loading against the limit, and the lines overloaded. A frame is the chart up to its
state, so the GIF grows a step at a time. The same plan is also a
[sheet](../renders/power_grid.png), the whole plan on one figure, with the action that produced
