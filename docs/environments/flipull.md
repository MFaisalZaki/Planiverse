...
fewest blocks it can be reduced to under the rules above is exactly the cartridge's target, which
means every shipped stage has a reachable goal, at the cost of a set that is ours rather than the
console's.
...
does not appear at all and the branching factor is between 1 and 3. A planner is never handed a
self-loop to close, at the cost of one successor evaluation per action to find out.
...
`str(state)` draws the board in the stage alphabet. `env.render()` prints the state history.
