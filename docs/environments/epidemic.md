...
[sheet](../renders/epidemic_chart.png), the whole plan on one figure, with the action that produced
...
The chart shows the infectious, the hospital load against the beds, the deaths against the target and the disruption points against the budget, week by week. The readings each environment charts, and the panels they go on, are in
