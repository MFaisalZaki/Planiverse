...
BFWS's plan for instance `0`: 17 moves, drawn as the readings of each state over the plan. A
frame is the chart up to its state, so the GIF grows a step at a time. The same plan is also a
[sheet](../renders/traffic_chart.png), the whole plan on one figure, with the action that produced
...
The chart shows the vehicles on the road, halted, arrived and still to come, and the
vehicle-seconds against the target, decision by decision. The plan is the accepted timer
with one early switch of its own, and it clears the grid with a hundred vehicle-seconds
less travel than the timer; the search took 23 expansions once the state carried the
queue, and ran out of a fifteen-minute budget without it. The readings each environment charts, and the panels they go on, are in
