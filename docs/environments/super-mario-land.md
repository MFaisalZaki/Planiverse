...
arc, fitted to the moving jump, because that is the one that decides what is reachable, at the
cost of a standing jump that reaches higher here than on the console.
...
`str(state)` draws the level in the alphabet above. `render_trace` typesets it into a contact
