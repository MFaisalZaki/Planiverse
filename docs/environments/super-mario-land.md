...
- **Import:** `from planiverse.environments.games.super_mario_land import SuperMarioLandGame`
- **Source:** [`planiverse/environments/games/super_mario_land.py`](../../planiverse/environments/games/super_mario_land.py)
- **Instances:** 24 levels, indices `0` to `23`: the twelve on the ramp, then twelve the generator drew, plus any you supply yourself
- **Generator:** `generate_instance(seed, width=None, gaps=None, platforms=None, enemies=None, hazards=None)`; see [Generating levels](#generating-levels)
- **Dependencies:** none
...
Indices `12` to `23` are levels the generator drew (`GENERATED_LEVELS`), each to the shape of one
on the ramp, with the seed beside it in the module and what it cost BFWS(w=2) in
`GENERATED_EXPANSIONS`, the same currency as `MEASURED_EXPANSIONS`.
...
|---|---|---|
| `width`, `height` | a bundled one's | the level, in tiles |
| `gaps` | a bundled one's | gaps of two or three tiles cut into the floor |
| `platforms` | a bundled one's | runs of three to six solid tiles, two or three tiles above the floor |
| `enemies` | a bundled one's | enemies walking on the floor |
| `hazards` | a bundled one's | single deadly tiles set into the floor |
...
Left unset, the layout options come from one of the twelve shipped levels drawn at random
(`PROFILES`): its length, and how many gaps, platforms, enemies and hazards it holds, so a
generated level is as long and as busy as one on the ramp. The method is generate-and-test,
which the procedural content generation literature calls search-based PCG (Togelius,
Yannakakis, Stanley and Browne, 2011, https://doi.org/10.1109/TCIAIG.2011.2148116; Shaker,
Togelius and Nelson, *Procedural Content Generation in Games*, 2016, https://pcgbook.com/); the
test is BFWS(w=2)
(Lipovetzky and Geffner, 2017, https://ojs.aaai.org/index.php/AAAI/article/view/11027).
...
| Path | What |
|---|---|
| [`super_mario_land.py`](../../planiverse/environments/games/super_mario_land.py) | `SuperMarioLandGame`, `SuperMarioLandState`, `SuperMarioLandAction` |
