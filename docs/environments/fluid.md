...
[contact sheet](../renders/fluid.png), every frame on one image, captioned with the step
...
Every frame is the state's own text: the cave, with the earth dug and the water where the ticks left it. See [docs/rendering.md](../rendering.md) for the other output formats.
