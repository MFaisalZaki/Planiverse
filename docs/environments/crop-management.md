...
node, and the [power grid](power-grid.md) is wide and shallow. This one is narrow, deep, and
blind until the end.

## Determinism

Verified rather than assumed: the same schedule in the same year produces the same yield to
the last digit. The schedule tuple is therefore the whole state — `__eq__` and `__hash__` are
