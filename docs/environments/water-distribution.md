...
[sheet](../renders/water_network_chart.png), the whole plan on one figure, with the action that produced
...
env.render_trace(trace, "water_network_chart.png", actions=result.plan, env=env)   # contact sheet
```
