...
BFWS's plan for instance `0`: 14 moves, one frame per state. The same plan is also a
[contact sheet](../renders/lemmings.png), every frame on one image, captioned with the step
...
Every frame is the state's own text: the level, the lemmings on it, and the skills, saved and lost counts. See [docs/rendering.md](../rendering.md) for the other output formats.
