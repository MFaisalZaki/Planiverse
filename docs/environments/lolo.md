...
`str(state)` draws the board in the glyphs listed under [The rules](#the-rules). `render_trace`
typesets it into a contact sheet, PDF, GIF or directory of PNGs:

```python
from planiverse.planners.width import IteratedBFWS
from planiverse.benchmark import measures

result = IteratedBFWS(max_width=1000, progress=measures.lolo).solve(game)
trace = game.simulate(result.plan)

game.render_trace(trace, "lolo.gif")                                 # animated
game.render_trace(trace, "lolo.png", actions=result.plan, env=game)  # contact sheet
```
