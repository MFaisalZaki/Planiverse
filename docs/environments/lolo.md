...
|---|---|---|
| `hearts` | a bundled one's | heart framers, of which `magic_hearts` (default 0) are magic |
...
Left unset, the inventory comes from one of the cartridge's rooms this module is exact for,
drawn at random (`PROFILES`, one per room in `EXACT_ROOMS`): its hearts and how many are magic,
its Emerald Framers, Snakeys and Medusas, and the shares of the board that are rock and tree,
so a generated room is stocked like a real one. The method is generate-and-test, which the
...
*Procedural Content Generation in Games*, 2016, https://pcgbook.com/); the test is
...
| Path | What |
|---|---|
| [`lolo.py`](../../planiverse/environments/games/lolo.py) | `LoloGame`, `LoloAction`, `EXACT_ROOMS` |
