...
population and its parts against the target, the funds, and the score, year by year. A frame is
the chart up to its state, so the GIF grows a step at a time. The same plan is also a
[sheet](../renders/micropolis.png), the whole plan on one figure, with the action that produced
