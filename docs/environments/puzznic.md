...
embedded in the module. Indices are `0`–`49` and stable, since they are a literal list, not a
directory listing.
...
Note that a level holding a single block of some colour is *born* terminal, so it ignores
every action; this is worth knowing when hand-writing a level to test against.
...
and after the step instead. Since blocks are identified by colour *and* position, a block that
...
- **`_check_and_remove_matches_` asserts `len(to_remove) != 1`.** Reaching a state where exactly one
  ...
  found symmetrically, so this should not trigger, but it is an assert on a hot path, not a guard.
