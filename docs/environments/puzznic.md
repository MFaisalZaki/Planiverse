...
included. The cost of that completeness is a rule set we wrote ourselves rather than one read off
the cartridge, so where it disagrees with the console the cartridge is right; see
[Levels](#levels).
...
same-type pair, whereas the cartridge leaves some such pairs untouched. Neither affects whether a
level is solvable here, but both mean this environment is a benchmark in its own right rather than
a prediction of the console.
