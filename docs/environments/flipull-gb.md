...
![BFWS solving flipull_gb instance 0](../renders/flipull_gb.gif)

BFWS's plan for instance `0`: 44 moves, one frame per state. The same plan is also a
[contact sheet](../renders/flipull_gb.png), captioned with the step number, the action that
