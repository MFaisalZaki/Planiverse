...
BFWS's plan for instance `0`: 36 moves, one frame per state. The same plan is also a
[contact sheet](../renders/factory.png), every frame on one image, captioned with the step
...
Every frame is the state's own text: the tick, where the character stands and what it holds,
each machine with its status and fuel, and the plates made and brought back. The plan is not
the scripted line the patch was accepted on (eighteen steps, two lines fuelled once and the
plates taken at the end): BFWS drips coal into the machines a piece at a time, empties the
first furnace six times while it waits, digs ore by hand and feeds it to the second furnace,
and reaches the twenty-six plates that way in thirty-six steps, after 71,727 expansions. See
