...
so the unachieved-goal count that SIW and BFWS classically lean on does not exist:
...
different answers and a planner that treats them as the same is not usable.
...
board with one block of a colour left, which can never be matched. The position is a dead
...
width, the way IW does. That gives each round IW's bounded frontier, so it is cheap, while the
