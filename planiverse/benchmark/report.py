...
#: Column headings for the LaTeX outcome table, short enough to fit a two-column page.
_STATUS_HEADERS = {"SOLVED": "Solved", "INVALID": "Invalid", "UNSOLVED": "Unsolved",
                   "TIMEOUT": "Time", "NODEOUT": "Exp.", "MEMOUT": "Mem.",
                   "ERROR": "Error", "UNSUPPORTED": "Unsup.", "MISSING": "Missing"}

#: The three statuses the harness itself imposed, grouped under one spanning header.
_BUDGET_STATUSES = ("TIMEOUT", "NODEOUT", "MEMOUT")


def latex_statuses(summary, fold_missing=False):
    """The outcome table as a LaTeX tabular, ready to \\input.

    Emits a column for every status that any planner actually recorded, so each row sums to
    that planner's task count. Dropping a status here is how a table stops adding up, which
    is why the columns come from the data rather than from a list written out by hand.

    `fold_missing` counts MISSING runs (i.e., ones that never happened, so the harness has no
    result file for them) as UNSOLVED. That is what a paper reporting a single denominator
    wants, since it keeps every row on the same total and never credits a planner with a run
    that did not happen. It is off by default: on a working sandbox MISSING means the
    experiment is incomplete, and silently renaming it to UNSOLVED would hide that.
    """
    rows = sorted(summary["coverage"], key=lambda r: (-r["solved"], r["planner"]))
    counts = {}
    for row in rows:
        statuses = dict(row["statuses"])
        if fold_missing and statuses.get("MISSING"):
            statuses["UNSOLVED"] = statuses.get("UNSOLVED", 0) + statuses.pop("MISSING")
        counts[row["planner"]] = statuses

    present = [status for status in STATUSES
               if any(counts[row["planner"]].get(status) for row in rows)]
    best_solved = max((row["solved"] for row in rows), default=None)
    medians = [row["median_seconds"] for row in rows if row["median_seconds"] is not None]
    best_median = min(medians) if medians else None

    lines = ["% Generated by planiverse-bench report. Needs \\usepackage{booktabs}.",
             "\\begin{tabular}{l" + "r" * (len(present) + 1) + "}", "\\toprule"]

    # The budget statuses sit next to each other in STATUSES, so the group header spans one
    # contiguous run of columns and needs no reordering here.
    budget = [status for status in present if status in _BUDGET_STATUSES]
    if budget:
        first = present.index(budget[0]) + 2
        lines += ["& " * (first - 1) + f"\\multicolumn{{{len(budget)}}}{{c}}{{Out of budget}}"
                  + " &" * (len(present) + 3 - first - len(budget)) + " \\\\",
                  f"\\cmidrule(lr){{{first}-{first + len(budget) - 1}}}"]

    lines += ["Planner & " + " & ".join(_STATUS_HEADERS.get(s, s.title()) for s in present)
              + " & Median (s) \\\\", "\\midrule"]

    for row in rows:
        cells = []
        for status in present:
            value = counts[row["planner"]].get(status, 0)
            cells.append(f"\\textbf{{{value}}}"
                         if status == "SOLVED" and value == best_solved else str(value))
        median = row["median_seconds"]
        cells.append("--" if median is None else
                     (f"\\textbf{{{median:.1f}}}" if median == best_median
                      else f"{median:.1f}"))
        lines.append(_escape(row["planner"].upper()) + " & " + " & ".join(cells) + " \\\\")
