...
from planiverse.environments.generation import (
    from_profile, interior, rng, scatter, solvable_draw, walled_grid,
)
...
def profile(level):
    ...
    the interior size, the number of colours, how many blocks each colour has, and the share
    of the interior that is wall."""
    rows = level.split("\n")
    width = max(len(row) for row in rows) - 2
    inside = [cell for row in rows[1:-1] for cell in row[1:-1].ljust(width)]
    counts = Counter(cell for cell in inside if cell in "123456789")
    return {"width": width, "height": len(rows) - 2, "colours": len(counts),
            "walls": round(inside.count("#") / len(inside), 3),
            "blocks_per_colour": tuple(sorted(counts.values()))}


#: The shape of each cartridge round: what `generate_instance` draws a level's options from
#: when the caller leaves them unset.
PROFILES = tuple(profile(level) for level in CARTRIDGE_LEVELS)

#: The largest round, in blocks, the generator draws shapes from by default: a shape the
#: breadth-first check decides within its budget. The cartridge's rounds run to over
#: thirty blocks, whose shapes a small search cannot decide at all.
MAX_BLOCKS = 10
...
class PuzznicGame(Environment):
    ...
    def generate_instance(self, seed=None, width=None, height=None, colours=None, walls=None,
                          blocks_per_colour=None, max_blocks=MAX_BLOCKS, solvable=True,
                          min_plan_length=4, search_limit=20_000, attempts=200):
        ...
        each draw takes the ones left unset from the profile of a cartridge round chosen at
        random (`PROFILES`), so a generated level has the shape of a real one. The rounds
        drawn from are those with at most `max_blocks` blocks, because the breadth-first
        check cannot decide the larger rounds' shapes within `search_limit`; `None` draws
        from every round. With `solvable` each draw is searched for up to `search_limit`
        ...
        profiles = tuple(p for p in PROFILES
                         if max_blocks is None or sum(p["blocks_per_colour"]) <= max_blocks)

        def draw(attempt):
            options = from_profile(random_, profiles, width=width, height=height,
                                   colours=colours, walls=walls,
                                   blocks_per_colour=blocks_per_colour)
