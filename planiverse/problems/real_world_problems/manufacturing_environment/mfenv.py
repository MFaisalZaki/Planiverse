...
class ActionType(Enum):
    ...
    FINISH_PRODUCTION_ALL = 2
    FINISH_PRODUCTION_CFG = 3


class ConfigurationAction:
    def __init__(self, cfg_id, action_type:ActionType):
        self.cfg = cfg_id
...
class MfgState:
    ...
    def __update__(self):
        ...
        self.literals = frozenset([f'{k}({str(v)})' for k, v in self._state.items()])
    ...
    def buy_cfg(self, cfg_id: int):
        ...
        # calculate reward
        reward = -1.0 * ret_state["market_incurring_costs"][cfg_id]

        # buy new configuration
        # update inucrred costs
        ret_state["configuration_costs"][cfg_id]["incurred_costs"] = ret_state["market_incurring_costs"][cfg_id]

        # update recurring costs
        ret_state["configuration_costs"][cfg_id]["recurring_costs"] = ret_state["market_recurring_costs"][cfg_id]
        ret_static_state["recurring_costs"] = ret_state["market_recurring_costs"][cfg_id]

        # update production rates
        ret_state["configuration_costs"][cfg_id]["production_rates"] = ret_state["market_production_rates"][cfg_id]
        ret_static_state["production_rates"] = ret_state["market_production_rates"][cfg_id]

        # update setup times
        ret_state["configuration_costs"][cfg_id]["setup_times"] = ret_state["market_setup_times"][cfg_id]

        # update cfgs status
        ret_state["configuration_costs"][cfg_id]["cfgs_status"] = (1 / ret_state["market_setup_times"][cfg_id])
...
class MfgEnv(RealWorldProblem):
    ...
    def successors(self, state):
        ...
        # 3. or continuing production for all configurations
