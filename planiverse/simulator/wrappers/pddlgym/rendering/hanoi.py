...
def get_objects_from_obs(obs):
    ...
    while len(discs):
        for d1 in discs:
            ...
            # Vendored change: was `import ipdb; ipdb.set_trace()`. ipdb is not a
            # dependency, so that raised ModuleNotFoundError and hid the real problem --
            # no disc is orderable, so `disc_pair_order` describes a cycle.
            raise ValueError("hanoi: the disc ordering in this state is cyclic")
