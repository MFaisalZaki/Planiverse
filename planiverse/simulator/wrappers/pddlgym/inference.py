...
from planiverse.simulator.wrappers.pddlgym.structs import Literal, LiteralConjunction, ground_literal
from planiverse.simulator.wrappers.pddlgym.utils import get_object_combinations
import functools
    ...
    def get_possible_assignments(self, variable, established_assignments, goal_literals, verbose=False):
        ...
        for goal_literal in goal_literals:
            ...
            if goal_literal.is_negative:
                # Vendored change: dropped an `if verbose: import ipdb; ipdb.set_trace()`
                # debug hook here; ipdb is not a dependency, so it only ever raised.
