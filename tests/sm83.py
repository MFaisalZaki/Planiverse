...
Only exists to build the synthetic cartridge in `counter_rom.py`, which is what makes the
generic Game Boy environment testable without any commercial ROM. It covers the instruction
forms that cartridge uses and nothing else: unsupported mnemonics raise rather than silently
assembling to the wrong bytes.
